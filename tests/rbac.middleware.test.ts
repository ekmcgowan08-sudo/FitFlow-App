...
// Unit tests for src/rbac/rbac.middleware.ts: requireRole, requireAllRoles,
...
describe("requireCoachOfClient", () => {
  ...
  it("lets an ADMIN through without querying the coach/client relationship", async () => {
    ...
    expect(next).toHaveBeenCalledWith();
    expect(prismaMock.coachAssignment.findUnique).not.toHaveBeenCalled();
  });
  ...
  it("allows a coach with an active assignment to the client", async () => {
    ...
    expect(prismaMock.coachAssignment.findUnique).toHaveBeenCalledWith({
      where: {
  ...
  it("forbids a coach whose assignment to the client is inactive", async () => {
    prismaMock.coachAssignment.findUnique.mockResolvedValueOnce({ relationshipStatus: "ended" });
    const next = makeNext();
    const req = makeReq({
      user: makeUser({ id: "coach-1", roles: ["COACH"] }),
      params: { clientId: "client-1" },
