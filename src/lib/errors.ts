...
// every failure in the API maps to one predictable HTTP status and one
// predictable JSON shape. Repositories translate Prisma errors into these
// (see lib/domain-errors.ts) so routes never need to know about Prisma
// error codes, and auth/RBAC failures use them directly.
 ...
 *
 * Every route throws AppError subclasses and forwards them with
 * `next(err)` rather than constructing ad-hoc `res.status(...)` calls, so
 * the status code / JSON body contract is defined in exactly one place.
 */
