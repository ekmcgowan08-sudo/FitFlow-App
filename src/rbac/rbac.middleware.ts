...
//
// All guards assume `authenticate` (see auth/auth.middleware.ts) has
...
import { AuthenticatedRequest, isAuthenticated, hasRole } from "../auth/types";
...
export function requireCoachOfClient(paramName = "clientId") {
  return async function requireCoachOfClientMiddleware(
    ...
    const relationship = await prisma.coachAssignment.findUnique({
      where: {
    ...
    if (!relationship || relationship.relationshipStatus !== "active") {
      return next(
