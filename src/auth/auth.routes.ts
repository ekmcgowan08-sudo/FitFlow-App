...
// rotation service (auth/token.service.ts) and rate limiters
...
router.post(
  ...
  async (req: Request, res: Response, next: NextFunction) => {
    try {
      ...
      // Default role is looked up/created by code, never taken from the
      // request body — a client can't self-assign ADMIN or COACH at
      // registration time.
...
router.post(
  ...
  async (req: Request, res: Response, next: NextFunction) => {
    try {
      ...
      // see auth/token.service.ts for the full rotation + reuse-detection
      // logic.
...
router.post("/auth/logout", async (req: Request, res: Response, next: NextFunction) => {
  try {
    ...
    // idempotency note in auth/token.service.ts.
    if (isNonEmptyString(refreshToken)) {
