...
// (prisma/schema.prisma). Used by auth.routes.ts for login, refresh, and
// logout.
//
// Design:
// - Access tokens are short-lived signed JWTs (see lib/token.config.ts).
...
import { JWT_ACCESS_SECRET, JWT_ISSUER, JWT_AUDIENCE, REFRESH_TOKEN_PEPPER } from "../lib/env";
import { ACCESS_TOKEN_TTL_SECONDS, REFRESH_TOKEN_TTL_DAYS } from "../lib/token.config";
