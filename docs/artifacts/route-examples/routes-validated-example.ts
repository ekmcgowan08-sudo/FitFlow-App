...
import { routeSchemas } from './validation-schemas';
...
router.get('/me', authMiddleware, validate(routeSchemas.me), async (req, res) => {
...
router.get('/members/:memberId/dashboard', authMiddleware, validate(routeSchemas.memberDashboard), async (req, res) => {
...
router.get('/members/:memberId/nutrition/logs', authMiddleware, validate(routeSchemas.nutritionList), async (req, res) => {
  const { memberId } = req.params;
...
router.post('/members/:memberId/nutrition/logs', authMiddleware, validate(routeSchemas.nutritionCreate), async (req, res) => {
  const { memberId } = req.params;
...
router.get('/members/:memberId/workout-plans', authMiddleware, validate(routeSchemas.workoutPlansList), async (req, res) => {
  const { memberId } = req.params;
...
router.post('/members/:memberId/workout-plans', authMiddleware, requireRole('coach', 'admin'), validate(routeSchemas.workoutPlansCreate), async (req, res) => {
  const { memberId } = req.params;
  const created = await prisma.workoutPlan.create({ data: { memberId, title: req.body.title, coachSource: req.body.coachSource || 'ai' } });
  res.status(201).json({ data: created });
});

router.get('/members/:memberId/meal-plans', authMiddleware, validate(routeSchemas.mealPlansList), async (req, res) => {
  const { memberId } = req.params;
  const data = await prisma.mealPlan.findMany({ include: { meals: true }, where: { memberId } });
  res.json({ data });
});

router.get('/members/:memberId/grocery-plans', authMiddleware, validate(routeSchemas.groceryPlansList), async (req, res) => {
  const { memberId } = req.params;
  const data = await prisma.groceryPlan.findMany({ where: { memberId }, include: { items: true } });
  res.json({ data });
});

router.get('/exercises', authMiddleware, validate(routeSchemas.exercisesList), async (req, res) => {
...
router.post('/gyms/check-ins', authMiddleware, validate(routeSchemas.gymCheckinCreate), async (req, res) => {
...
router.get('/members/:memberId/achievements', authMiddleware, validate(routeSchemas.achievementsList), async (req, res) => {
  const { memberId } = req.params;
...
router.post('/members/:memberId/coach-assignments', authMiddleware, requireRole('admin'), validate(routeSchemas.coachAssignmentCreate), async (req, res) => {
  const { memberId } = req.params;
